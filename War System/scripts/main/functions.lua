...
-- Обработчик ошибок (написать).
	...
	local function Loop()
		...
		for x2 = 1, length do
			for y2 = 1, width do
				if x2 == length or y2 == width or x == 1 or y2 == 1 then
					for i, obj in pairs(supposedObjects) do
						local x3, y3 = x + math.floor(x3 - 0.5*length), y + math.floor(y3 - 0.5*width);
		...
		for property, value in pairs(func.PropertiesToTable(object(projTab.textureTab.name))) do
			...
			if projTab.textureTab[property] and tzodTextureTab[property] ~= projTab.textureTab[property] then 
		...
		x, y = x1, y1;
		n = n + 1;
		pushcmd(Loop, projTab.speed)
	end;
	local function Hit(objLink, obj)
		local projTab = level.projectiles[projName];
		local splash;
		damage(projObj.damage, obj)
		if obj == "tank" then
			func.tank.OnDamage(obj, projTab)
		end;
		if obj == "crate" or obj == "tank" then
			pushobj(objLink, projTab.dir, projTab.pushPower)
		end;
