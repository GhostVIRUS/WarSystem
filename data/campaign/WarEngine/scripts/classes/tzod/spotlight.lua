...
-- declaring
dbg.Print("| Requiring 'Spotlight' class.")
Sprite = objects.Class("Spotlight", Entity)

-- public methods
function Spotlight:initialize(--[[name,]] pos, props)
	Entity:initialize(self, --[[name,]] pos, props)

	self._objectType = "respawn_point"
end

function Spotlight:follow(whoName, speed, iteration, copyDir) -- speed here and above in px/sec
	if iteration == 0 then
