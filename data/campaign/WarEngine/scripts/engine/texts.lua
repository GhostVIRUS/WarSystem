...
local function LoadFile(file, lang)
	...
	local textsList;
	local isLoaded, errorMsg = pcall(function() textsList = dofile(const.txtPath..lang.."/"..file..".txt") end);
	...
	elseif not textsList and not errorMsg then -- if file loaded without errors, but returns nothing
	...
	if isLoaded then
		...
		dbg.Print("| WARNING: Text file '"..file.."' of language '"..lang.."' wasn't loaded: "..errorMsg, "engine");
		return false;
	end;
end;
...
--	package.path = "data/"..const.txtPath..lang.."/info.lua";
	...
	local isLoaded, errorMsg = pcall(function() info = dofile(const.txtPath..lang.."/info.lua") end);
...
--	package.path = "data/"..const.txtPath..lang.."/?.txt";
	...
	if isLoaded then
		...
		dbg.Print("| WARNING: Language '"..lang.."' wasn't loaded: "..errorMsg, "engine");
		return false;
	end;
end;
